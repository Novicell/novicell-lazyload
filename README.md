...
First include the js file in your js bundle and in your HTML:
...
```html
    <script src="/dist/scripts/lazy.min.js"></script>
```

## Implementation
This script *lazyloads* by swapping the `data-src` or `data-srcset` to an actual `src` or `srcset`.
For all implementations you should have a `lazyload`-class and `data-query-obj` on the image. Everything inside the `data-query-obj` is general settings that is applied on every src in the srcset.
